...
# train_loader = torch_geometric.loader.DataLoader(train_dataset, batch_sampler = BatchSampler_train, num_workers = num_workers)
# val_loader = torch_geometric.loader.DataLoader(val_dataset, batch_sampler = BatchSampler_val, num_workers = num_workers)
...
# model.lo (best_state_dict)
model.load_state_dict(torch.load('results_RSA_chiro/best_model.pt'))
model.to(device)
...
targets, outputs = evaluate_classification_loop_alpha(model, test_loader, device, batch_size = 1000, dataset_size = len(test_dataset), is_binary=False)
